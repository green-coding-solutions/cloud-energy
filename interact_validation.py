...
X = df_new[[ 'HW_MemAmountGB', 'TDP', 'utilization', 'CPUCores', 'CPUThreads', 'HW_CPUFreq', 'Hardware_Availability_Year', 'HW_FormFactor', 'HW_Vendor']]
...
print("[Interact DC Original (untuned)] K-fold CV score range: %.2f < %.2f < %.2f" % (kf_cv_scores.min(), kf_cv_scores.mean(), kf_cv_scores.max()) )
...
kfold = RepeatedKFold()
kf_cv_scores = cross_val_score(model, X, y, cv=kfold, scoring="neg_mean_absolute_error")
print("[Interact DC cloud available variables (untuned)] K-fold CV score range: %.2f < %.2f < %.2f" % (kf_cv_scores.min(), kf_cv_scores.mean(), kf_cv_scores.max()) )


X = df_new[[ 'HW_MemAmountGB', 'TDP', 'utilization', 'CPUCores', 'HW_CPUFreq', 'Hardware_Availability_Year', 'Architecture', 'CPUMake']]
